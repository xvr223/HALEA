...
// ── Color Helpers ── shared pixel pipeline lives in @/lib/grade ──────────────
...
${ctx}`
  ...
  // shared loader: HALEA Code input box + Look Library hand-off both land here
  const loadCode = (text: string): boolean => {
    const res = decodeGrade(text)
    if (!res) return false
  ...
  const importHaleaCode = () => {
    if (!loadCode(codeInput)) { toast('Kode tidak valid — cek lagi', 'err'); return }
    setCodeInput('')
    toast('✦ Look dimuat dari HALEA Code!')
  }

  // Look Library hand-off: /looks stores the chosen code, we load it on arrival
  useEffect(() => {
    try {
      const c = sessionStorage.getItem('halea_use_code')
      if (c) {
        sessionStorage.removeItem('halea_use_code')
        if (loadCode(c)) toast('🌍 Look dari Library dimuat — upload footage & export')
      }
    } catch {}
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  // Publish the current look to the community Look Library (free — supply side)
  const [publishing, setPublishing] = useState(false)
  const publishLook = async () => {
    if (!authUser) { toast('Daftar gratis dulu untuk publish ✦', 'warn'); router.push('/login?next=/studio'); return }
    if (!nodes.length) { toast('Match Colors / Prompt AI dulu — belum ada look', 'warn'); return }
    setPublishing(true)
    try {
      const code = encodeGrade(nodes, lutName)
      const res = await fetch('/api/looks', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: lutName || 'HALEA Look', code, author: authUser.name }) })
      const j = await res.json()
      if (!j.ok) throw new Error(j.error || 'fail')
      try { localStorage.setItem('halea_m_publish', '1') } catch {}
      toast('🌍 Look terpublish ke Library — buka /looks buat lihat!')
    } catch { toast('Gagal publish — coba lagi', 'err') }
    setPublishing(false)
  }
    ...
    <div className="hidden md:flex h-[calc(100vh-56px)] overflow-hidden">
      ...
      <div className="w-64 flex-shrink-0 border-l border-b1 flex flex-col">
        ...
        <div className="flex-1 p-4 flex flex-col gap-4 overflow-y-auto">
          ...
          {/* Publish to Look Library */}
          {nodes.length>0&&(
            <button onClick={publishLook} disabled={publishing}
              className="w-full py-2.5 rounded-xl text-[11px] font-bold border border-ok/30 bg-ok/10 text-ok hover:bg-ok/20 transition-colors flex items-center justify-center gap-1.5 disabled:opacity-40">
              {publishing
                ? <><span className="w-3 h-3 border-2 border-ok/30 border-t-ok rounded-full animate-spin"/>Publishing...</>
                : <>🌍 Publish ke Look Library <span className="text-[8px] opacity-70">gratis</span></>}
            </button>
          )}
      ...
      <div className="flex-1 overflow-hidden relative">
        ...
        <div className={`absolute inset-0 overflow-y-auto transition-opacity duration-200 ${mobileTab==='export'?'opacity-100 pointer-events-auto':'opacity-0 pointer-events-none'}`}>
          <div className="p-5 flex flex-col gap-5" style={{paddingBottom:'120px'}}>
            ...
            {nodes.length>0&&(
              <button onClick={publishLook} disabled={publishing}
                className="w-full py-4 rounded-2xl text-sm font-bold border border-ok/30 bg-ok/10 text-ok active:scale-[0.97] transition-all flex items-center justify-center gap-2 disabled:opacity-40">
                {publishing
                  ? <><span className="w-4 h-4 border-2 border-ok/30 border-t-ok rounded-full animate-spin"/>Publishing...</>
                  : <>🌍 Publish ke Look Library — gratis</>}
              </button>
            )}
